...
Do not delete the entire `com.example.DockCat` defaults domain as a first recovery step. Doing so would also remove DockCat preferences such as enablement, notification behavior, animation, source options, and display calibration. The recovery URLs never perform a preference reset.
