...
final class CatAnimationAtlasLoaderTests: XCTestCase {
    ...
    static func manifest() -> CatAnimationAtlasManifest { Self.valid(clips: CatAnimationClipID.allCases.map { id in Self.clip(id, frames: id == .wake ? ["wake_0", "wake_1"] : ["\(id.rawValue)_0"]) }) }
    static func valid(clips: [CatAnimationClipManifest]) -> CatAnimationAtlasManifest { .init(schemaVersion: 1, assetSetID: "dockcat-test", assetSetVersion: "1", atlasName: "TestCat", logicalCanvasSize: Size(width: 150, height: 110), nativeScale: 2, anchors: .init(visualAnchor: Point(x: 75, y: 35), feetAnchor: Point(x: 75, y: 35), carryAnchor: Point(x: 117, y: 73), handoffSize: Size(width: 36, height: 24)), orientationPolicy: .canonicalRightFacingMirrorLeftRotateVertical, clips: clips) }
...
private struct FixtureLocator: CatAnimationAssetLocating {
    let manifest: CatAnimationAtlasManifest?; var wrongSizeFrame: String? = nil; var missingFrame: String? = nil
    ...
    func sourceImageMetadata(for frameName: String) throws -> CatFrameImageMetadata { frameName == wrongSizeFrame ? .init(pixelWidth: 299, pixelHeight: 220) : .init(pixelWidth: 300, pixelHeight: 220) }
}
